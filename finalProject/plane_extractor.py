...
def extract_ceil(pointcloud, show_progress=False, voxel_size=0.1, nb_neighbors=30, std_ratio=2.0, z_ratio=0.25,
                  save_filename=None):
    if show_progress:
...
def extract_floor(pointcloud, show_progress=False, voxel_size=0.1, nb_neighbors=30, std_ratio=2.0, z_ratio=0.5,
                  save_filename=None):
    if show_progress:
        o3d.visualization.draw_geometries([pointcloud])
    ...
    points = np.array(segments.points)
    # segments.colors = o3d.utility.Vector3dVector(np.tile(color, reps=[np.asarray(segments.points).shape[0], 1]))
    if save_filename != None:
        np.savetxt(save_filename, points[:, :2])
